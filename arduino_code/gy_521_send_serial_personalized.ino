...
//define char arry for byte array
char ax[sizeof(float)], ay[sizeof(float)], az[sizeof(float)];
byte buttonSt;
...
{      
  ...
  */
  ...
  set_last_read_angle_data(millis(), 0, 0, 0, 0, 0, 0);
    ...
    //when button push, it starts to detect
...
//  float accel_vector_length = sqrt(pow(accel_x,2) + pow(accel_y,2) + pow(accel_z,2));
  ...
  memcpy(ax, &angle_x, sizeof(float));
  memcpy(ay, &angle_y, sizeof(float));
  memcpy(az, &angle_z, sizeof(float));
